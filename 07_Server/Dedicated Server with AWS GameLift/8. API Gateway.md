...
- Resource 생성
	...
	- ![APIGateway_Resource_CreateMethod](/Image/Server/AWS_APIGateway_Resource_CreateMethod.png)
		...
		- lambda proxy integration ^dbc74c
...
- API Gateway $\to$ Resource $\to$ Get Method 의 실행 흐름 (Method Execution)
	...
	2. Integraion Request (통합 요청)
		...
		- 데이터 매핑 및 백엔드 호출을 진행
			...
			- 만약 [[8. API Gateway#^dbc74c||Lambda Proxy]] 가 꺼져 있다면, 여기서 템플릿을 사용해 데이터를 가공해서 넘겨준다.
	...
	3. Integration Response (통합 응답)
		...
		- 결과 해석 및 에러 매핑
			...
			- Regex(정규식)[^1] 을 사용하여 람다의 에러 문자열을 HTTP 상태 코드 로 매칭한다.
	4. Method Response (메서드 응답)
		- 클라이언트에게 최종적으로 전달될 HTTP 응답을 정의하는 계층
		- 응답 규격 정의 및 헤더 설정.
			- 클라이언트가 받게 될 최종 HTTP 상태 코드를 정의
			- CORS 헤더[^2]와 같이 브라우저나 클라이언트가 필요로 하는 응답 헤더를 최종적으로 붙인다.
			- 응답 데이터의 모델 (JSON 스키마) 을 정의하여 문서화
	- TEST
		- AWS 콘솔 내부에서 제공하는 API 시뮬레이더
		- 외부 툴 없이 API 로직을 즉시 검증한다. 실행 버튼을 누르면 위의 4단계(Method Req $\to$  Integ Req $\to$  Lambda $\to$  Integ Res $\to$  Method Res)를 실제로 수행하고, 각 단계별 로그(Logs)를 화면에 보여주어 디버깅에 필수적이다. 다음과 같이 Log 가 생긴다.
			```node.js
			/list-fleets - GET method test results
			Request
			/list-fleets
			Latency ms
			1666
			...
			200
			Response body
			{"$metadata":{"httpStatusCode":200,"requestId":"11935f40-a3a3-423d-8bb0-8b783ee6eb96","attempts":1,"totalRetryDelay":0},"FleetIds":[]}
			Response headers
			{
			  "Content-Type": "application/json",
			  "X-Amzn-Trace-Id": "Root=1-695018d4-d8c1a35aba77b9993522ea0c;Parent=2563814c35bfb607;Sampled=0;Lineage=1:82f8354c:0"
			}
			Logs
			Execution log for request 3b01edc4-2b6e-4329-9955-2b64972de2eb
			```
		- CloundWatch $\to$  Log Management $\to$ /aws/lambda/ListFleets 를 보면 위의 test 에 대한 로그가 생긴 것을 확인할 수 있다.

[^1]: 특정한 규칙을 가진 문자열의 패턴을 찾거나 가공하기 위해 사용하는 형식 언어이다. 지금은 람다 에러 -> HTTP 에러 변환용

[^2]: Cross-Origin Resource Sharing : 교차 출저 리소스 공유를 제어하는 HTTP 헤더. 브라우저의 보안 정책상, 다른 도메인에 있는 리소스를 함부로 가져오지 못하게 막혀 있는데, 서버가 허용해 주겠다고 명시적으로 알려주는 허가증이다. API Gateway 에서는 웹 브라우저 에서 실행되는 게임이나 관리자 페이지가 API 를 호출할 때 특정 헤더를 응답에 붙여주어야 에러가 나지 않는다.

- Stage (배포)
	- API Gateway $\to$ APIs $\to$ Resources - GameSessions $\to$  Resources 에서 Deploy API 버튼 클릭
	- Deploy API 창에서 Stage 의 이름을 설정하고 Deploy 버튼을 클릭한다. ![APIGateway_ResourceStage](/Image/Server/AWS_APIGateway_ResourceStage.png)
	- API Gateway $\to$ APIs $\to$ Resources - GameSessions $\to$ Stages 에 생성한 Stage 가 등록된다. 해당 Stage 에서 Invoke URL 을 얻을 수 있다.
	- Stages 탭은 배포가 완료된 라이브 상태의 API 정보가 있는 곳이다.
	- Stage 를 클릭하여 펼쳐서 특정 Method 를 선택하면 Invoke URL 을 볼 수 있다.
	- Invoke URL 의 구조
		- ` https://{API-ID}.execute-api.{Region}.amazonaws.com/{StageName}/{ResourceName} `
	- 언리얼 엔진에서 요청을 보낼 때는 API Gateway 메인 주소가 아니라 Resource 이름 까지 포함된 전체 주소 를 써야 한다.

##### Module

- ...\Source\DedicatedServers 폴더 생성. 해당 클라우드에 엑세스할 때 사용하는 코드와 같이 데디케이티드 서버를 위한 특정 코드들을 저장한다.
- 모듈을 위해서는 해당 모듈명으로된 .h, .cpp, .Build.cs 파일과 Private, Public 폴더가 필요하다.
...
IMPLEMENT_PRIMARY_GAME_MODULE( FDefaultModuleImpl, DedicatedServer, "DedicatedServer" );

```

```c#
// DedicatedServer.Build.cs
...
{
	"FileVersion": 3,
	"EngineAssociation": "{FDAA09A1-4F0C-96E1-27D7-C0ACB342F204}",
	"Category": "",
	"Description": "",
	"Modules": [
		{
...
- FPSTemplate.Target.cs ,FPSTemplateEditor.Target.cs, FPSTemplateSever.Target.cs 에  
	- 'extraModulenames.Add("DedicatedServer")' 추가.
- FPSTemplate.Build.cs 의 PublicDependencyModuleNames.AddRange 에 "DedicatedServer" 추가.


- DedicatedServer 에 GameMode 에 대한 Base 클래스를 두고 FPSTemplate 에서는 해당 Base 클래스를 상속 받아서 사용하도록 구조변경
	- DedicatedServer 프로젝트에 GameMode 클래스 추가.![DedicatedServer GameMode 추가.](/Image/Server/ADD_DedicatedServer_GameMode.png)
```c++
// DS_GameMode.h
...
UCLASS()  
class DEDICATEDSERVER_API ADS_GameMode : public AGameMode  
{  
    GENERATED_BODY()  
...
{  
    ...
    LogFiles.Add(TEXT("FPSTemplate_5_4_or_5_5/Saved/Logs/FPSTemplate.log"));  
...
- FPSTemplate 의 ShooterGameModeBase 의 부모 클래스를 DS_GameMode 로 변경한다.
- FPSTemplate 의 ShooterGameMode 클래스 삭제. (이제 ShooterGameModeBase 가 DSGamemode 로부터 GameLift 기능을 상속 받게 된다. $\to$ FPSTemplate 에서는 GameLift SDK 에 대한 의존성이 필요 없게 된다 $\to$ FPSTemplate.Build.cs 에서GameLiftServerSDK 의존성 부분을 삭제)
- Unreal Editor 에서 기존의 GameMode Blueprint 파일을 삭제하고 ShooterGameModeBase 를 상속 받는 BP 파일 생성 $\to$  생성된 BP_ShooterGameModeBase 파일을 프로젝트의 기본 GameMode 로 설정.

##### API Gateway endpoint test

1. Create HUD
	1. HUD 클래스 생성 ![Create_API_Test_UI_Class](/Image/Server/Create_API_Test_UI_Class.png)
	2. Overlay 용 UserWidget![Create_API_Test_Overlay_UserWidget](/Image/Server/Create_API_Test_Overlay_UserWidget.png)
	3. DedicatedServer.Build.cs 에 UMG 추가.
		```c++
		// DedicatedServer.Build.cs
		...
		...
		UCLASS()  
		class DEDICATEDSERVER_API AAPITestHUD : public AHUD  
		{  
		    GENERATED_BODY()  
		public:  
		...
		```
		4. WBP_APITestOverlay 생성![APITestOverlay1](/Image/Server/APITestOverlay1.png)
		5. BP_APITestHUD 생성 (APITestHUD 를 상속) $\to$ APITestOverLayClass 에 WBP_APITestOverlay 를 설정.
		6. 임시 Map 생성하여 GameMode Override 를 설정하고 HUD class 를 BP_APITestHUD 로 설정.
		7. ![APITestOverlay_OnScreen](/Image/Server/APITestOverlay_OnScreen.png)