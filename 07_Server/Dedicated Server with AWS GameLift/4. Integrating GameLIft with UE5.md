...
게임이 끝났을 때, 남겨진 기록(로그) 를 어디에서 가져가야 하는지 GameLift 에게 알려준다.
```c++
void AShooterGameMode::InitGameLift()
{
	...
...
```
- TArray\<FString> LogFiles; : 로그 파일 경로들을 담을 문자열 배열을 만든다. (로그 파일이 여러개일 수 있음)
- LogFiles.Add() : 로그 파일이 저장되는 경로를 배열에 추가.
- ProcessParameters.logParameters = LogFiles : 로그 주소를 FProcessParameters 에 저장.

- GameLift 서버는 클라우드(EC2) 위에서 돌아간다. 게임 세션이 끝나면 서버 인스턴스는 종료되고 사라질 수 있다.
- 서버가 사라지면 로그도 같이 사라지는데 나중에 원인을 파악할 수 없다 따라서 GameLift 는 세임 세션이 종료되는 순간, 여기에 적인 경로에 있는 파일들을 자동으로 긇어서 AWS 클라우드 (S3 등) 에 영구 저장해 준다.

##### ProgressReady

ProgressReady 는 초기화 코드의 가장 마지막에 호출한다.
이 함수를 호추할 때 지금까지 설정한 모든 정보가 담긴 FProcessParameter 를 GameLift 에 전달한다.
해당 호출은 포트 번호, 로그 경로, 종료 신호를 받을 함수 지정 등의 정보를 GameLift 서비스에 등록한다. GameLift 는 이 서버를 Active 상태로 인식하고 그때부터 플레이어를 매칭시켜 보내주기 시작한다.
OnHealthCheck 과 같은 콜백들이 이때부터 작동하기 시작한다.
...
- IDE 에서 'Development Editor', 'Development Server' 모두 빌드한다.

1. 패키징을 보관할 폴더 'Build' 를 프로젝트 루트 폴더에 추가한다.
2. 'Build' 폴더 내에 첫 번째 패키지를 저장할 'Build_1' 폴더를 추가한다.
3. Editor 에서 'Maps' 폴더에 'ShooterMap' Level 를 추가한다.
4. Project Editor 에서 Default Map 을 ShooterMap 으로 정하고 'List of maps to include in a packaged build' 에 'ShooterMap' 을 추가한다.
5. Dedicated Server 를 패키징
	1. 'Platform' 클릭 $\to$  'Windows' $\to$ 'Build Target' 에서 Server 빌드 타겟을 선택
	2. 'Platform' 클릭 $\to$ 'Windows' $\to$ 'Use Project Setting (Developmemt)' 선택
	3. 'Platform' 클릭 $\to$ 'Windows' $\to$ 'Package Project' 클릭 -> 'Build_1' 폴더 선택
	4. 1~3 까지의 과정으로 'Build_1' 폴더에는 'WindowServer' 폴더가 생성된다.
	5. "../WindowServer/FPSTemplate/Binaries/Win32" 폴더에 있는 'FPSTemplateServer..exe' 가 메인 파일이며 GameLift 에게 실행하라고 알려줘야 하는 파일이다.
6. 클라이언트 빌드 패키징
	1. 'Platform' 클릭 $\to$ 'Windows' $\to$ 'Build Target' 에서 클라이언트 빌드 타겟을 선택
	2. 'Platform' 클릭 $\to$ 'Windows' $\to$ 'Package Project' 클릭 -> 'Build_1' 폴더 선택
	3. 1~2 까지의 과정으로 'Builed_1' 폴더에는 'Window' 폴더가 생성된다.

##### Open SSl 빌드 파일 을 프로젝트 폴더에 추가.

- AWS 서버 SDK 가 서버 빌드에서 사용되므로, 실행파일이 있는 Windows Server 폴더에 [[1. Install GameLift Plugin#^28e7bf|준비해 둔 Open SSL 빌드 파일]]을 붙여 넣어야 한다.
- 준비해둔 Open SSL 빌드 파일 보관을 위해 프로젝트 루트 폴더에 'OpenSSLBinaries' 폴더 추가 후 Open SSL 빌드 파일 저장.
- "OpenSSLBinaries" 에 저장한 Open SSL 빌드 파일을 복사하여 "루트폴더\Build\Build_1\WindowsServer\FPSTemplate\Binaries\Win64" 에 붙여넣기 한다.